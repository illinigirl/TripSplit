...
// MARK: - Edit Line Item Sheet
struct EditLineItemSheet: View {
    @Environment(\.dismiss) private var dismiss
    let personName: String
    let personColor: String
    let item: TempLineItem
    let onSave: (TempLineItem) -> Void

    @State private var itemName: String
    @State private var itemAmount: String

    init(personName: String, personColor: String, item: TempLineItem, onSave: @escaping (TempLineItem) -> Void) {
        self.personName = personName
        self.personColor = personColor
        self.item = item
        self.onSave = onSave
        _itemName = State(initialValue: item.name)
        _itemAmount = State(initialValue: String(item.amount))
    }
...
// MARK: - Edit Shared Item Sheet
struct EditSharedItemSheet: View {
    @Environment(\.dismiss) private var dismiss
    let participants: [Person]
    let item: TempSharedItem
    let onSave: (TempSharedItem) -> Void

    @State private var itemName: String
    @State private var itemAmount: String
    @State private var selectedPeople: Set<PersistentIdentifier>
    @State private var isCustomSplit: Bool
    @State private var customShares: [PersistentIdentifier: Int]

    init(participants: [Person], item: TempSharedItem, onSave: @escaping (TempSharedItem) -> Void) {
        self.participants = participants
        self.item = item
        self.onSave = onSave
        _itemName = State(initialValue: item.name)
        _itemAmount = State(initialValue: String(item.amount))
        _selectedPeople = State(initialValue: Set(item.sharedByIDs))
        _isCustomSplit = State(initialValue: item.isCustomSplit)
        _customShares = State(initialValue: item.shares)
    }
    ...
    var body: some View {
        NavigationStack {
            ...
            .navigationTitle("Edit Shared Item")
            .navigationBarTitleDisplayMode(.inline)
            .toolbar {
                ToolbarItem(placement: .cancellationAction) {
                    Button("Cancel") { dismiss() }
...
// MARK: - Edit Item Identifiers
struct LineItemEditInfo: Identifiable {
    let id = UUID()
    let personID: PersistentIdentifier
    let itemIndex: Int
}

struct SharedItemEditWrapper: Identifiable {
    let id = UUID()
    let index: Int
}
